...
- A repo's git worktrees appear as `⎇ <name>` rows under the repo, not as separate projects, and expand into their own chats. The link back to the main checkout is read from the worktree's `.git` file, so it works no matter where the worktree lives on disk.
- Sessions whose directory no longer exists — retired worktrees, deleted temp dirs, moved projects — are dropped entirely. They are not hidden and there is no key to bring them back. The exception is a project on an unmounted volume (`/Volumes/…`, `/mnt/…`): it stays listed and reads `(volume not mounted)`, since plugging the drive back in restores it.
- `opt+x` hide and `opt+r` rename work on worktree rows as well as project rows.
